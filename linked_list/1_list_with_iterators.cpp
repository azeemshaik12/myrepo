// double linked list with custom iterator 
//still have errors for end iterator because list tail->next =null 
#include<iostream>
#include<algorithm>
#include<numeric>
#include<vector>
...
~d_list(){
 ...
 class Iterator{
     public:
     using itertor_category =std::bidirectional_iterator_tag;
...
int main(){
   ...
   // l.print_list();
    ...
    std::replace(j.begin(),j.end(),7,99);
    std::cout<<"\nprint j after replace ";
    std::for_each(j.begin(),j.end(),[](auto a){std::cout<<a<<",";});
     l.print_list();
     for(auto i:l){
         std::cout<<i<<",";
